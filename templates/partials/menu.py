...
class Menu(UserControl):
    ...
    def build(self):
        ...
        self.logo = Image(src_base64=self.imagem_para_base64(os.path.join(self.static_path,
                          'img/logo_pymazon_white.png')))
        self.location = Text('DF')
        self.div_location = Container(
            Column([Text('Delivetery to'), self.location], alignment=Alignment(0, 0)))
        self.category = Dropdown(width=200, options=[dropdown.Option('Books'), dropdown.Option('Electronics'), dropdown.Option(
            'Computers'), dropdown.Option('Home'), dropdown.Option('Garden'), dropdown.Option('Tools'), dropdown.Option('Grocery')], on_change=lambda _: print('changed'), label='All')
        self.search = TextField(hint_text='Search here',
                                suffix_icon=icons.SEARCH)
        self.profile = TextButton('Hello, User', disabled=True)
        self.btn_login_logout = TextButton(
            'Sign in', on_click=lambda _: self.page.go('/login'))
        self.div_profile = Column([self.profile, self.btn_login_logout])
        self.returns_orders = Text('Returns \n& Orders')
        self.cart = Container(TextButton(
            'Cart', on_click=lambda _: self.page.go('/cart'), icon=icons.SHOPPING_CART))
        return Container(Row(
            [Container(self.logo, expand=1), Container(self.div_location, expand=1), Container(self.category, expand=1), Container(
